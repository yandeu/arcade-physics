 ...
 */
export const GetOverlapX = (body1, body2, overlapOnly, bias) => {
  let overlap = 0
  ...
  if (body1._dx === 0 && body2._dx === 0) {
    ...
    if (
      ...
      if (body2.physicsType === CONST.PHYSICS_TYPE.STATIC_BODY && !overlapOnly) {
        body1.blocked.none = false
        body1.blocked.right = true
      }

      if (body1.physicsType === CONST.PHYSICS_TYPE.STATIC_BODY && !overlapOnly) {
